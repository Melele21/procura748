...
const STAGES = [
  ...
  { id:"CMP", label:"Comparación",     color:B.gold,    icon:"⚖️" },
...
const UNITS = ["Unidad","Kg","L","m","m²","m³","Caja","Pallet","Global","Servicio","Hora"];
...
const DISC_TYPES = ["Cantidad incorrecta","Ítem diferente","Daño físico","Doc. faltante","Calidad no conforme","Otro"];
const PAY_METHODS = ["Transferencia bancaria","Cheque","Efectivo","Débito directo","Otro"];

const DEMO_SUPPLIERS = [
  { id:"S001", name:"Suministros Técnicos C.A.",       contact:"Carlos Méndez", email:"cmendez@sumtec.com",  phone:"+58 212 555-0101", category:"Materiales",  rating:4.5, active:true },
  { id:"S002", name:"Ferretería Industrial del Norte", contact:"Ana López",     email:"alopez@finorte.com",  phone:"+58 212 555-0202", category:"Repuestos",   rating:4.0, active:true },
  { id:"S003", name:"Equipos y Soluciones S.A.",       contact:"Pedro Ruiz",    email:"pruiz@equipsol.com",  phone:"+58 212 555-0303", category:"Equipos",     rating:4.8, active:true },
  { id:"S004", name:"Transporte Rápido Express",       contact:"María Torres",  email:"mtorres@trex.com",    phone:"+58 212 555-0404", category:"Transporte",  rating:3.9, active:true },
  { id:"S005", name:"Consumibles Pro Venezuela",       contact:"Luis García",   email:"lgarcia@conspro.com", phone:"+58 212 555-0505", category:"Consumibles", rating:4.2, active:true },
];
...
.mod-title{font-family:'Montserrat';font-size:16px;font-weight:800;color:${B.carbon};}
...
function Dashboard({ ctx }) {
  ...
  const alerts=[
    ...
    ...cmps.filter(c=>c.status==="pendiente").map(c=>({msg:`Comparativo ${c.id} pendiente de aprobación`,color:B.goldD,icon:"⚖️",page:"CMP"})),
  ];
    ...
    <div className="fi">
      ...
      <div className="pb">
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:22 }}>
          {[
            { l:"Total POs",    v:total,                          c:B.carbon,  icon:"📋" },
            { l:"En proceso",   v:active,                         c:"#5B9BD5", icon:"🔄" },
            { l:"Completadas",  v:complete,                       c:"#5AAD7A", icon:"✅" },
            { l:"Total pagado", v:`$${totalPaid.toLocaleString()}`,c:B.goldD,  icon:"💰" },
          ].map(s=>(
        ...
        {alerts.length>0&&(
          <div className="card" style={{ padding:"16px 18px", marginBottom:20 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
              ...
              <div className="ct">Requiere atención</div>
            </div>
            {alerts.map((a,i)=>(
              <div key={i} onClick={()=>setPage(a.page)} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", background:`rgba(${a.color==="#D4745A"?"212,116,90":"168,134,58"},.06)`, border:`1px solid rgba(${a.color==="#D4745A"?"212,116,90":"168,134,58"},.2)`, borderRadius:7, marginBottom:6, cursor:"pointer", transition:"opacity .15s" }}>
                ...
                <span style={{ fontSize:11, color:B.gray3, fontFamily:"'Montserrat'", fontWeight:600 }}>Ver →</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="card" style={{ padding:"18px", marginBottom:20 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
            <div style={{ width:3, height:16, background:B.gold, borderRadius:2 }} />
            <div className="ct">Pipeline de procura</div>
          </div>
  ...
  return (
    <div className="fi">
      <div className="ph">
        ...
        <button className="btn btn-gold" onClick={()=>setShowForm(true)}>+ Nueva Requisición</button>
      </div>
      <div className="pb">
        <div className="gold-line" />
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          {[["Total PR",reqs.length,B.carbon],["En Requisición",reqs.filter(r=>r.stage==="REQ").length,"#7C72DC"],["Urgentes",reqs.filter(r=>r.priority==="urgente").length,"#D4745A"]].map(([l,v,c])=>(
        ...
        <div className="card" style={{overflow:"hidden"}}>
          ...
          {reqs.length===0?<Empty icon="📋" msg="Sin requisiciones" sub="Crea la primera para iniciar el proceso de compra" />:
          reqs.map(r=>{
            const s=STAGES.find(s=>s.id===r.stage); const p=PRIORITY[r.priority];
      ...
      {showForm&&<REQForm onClose={()=>setShowForm(false)} onSubmit={handleCreate} />}
      {selected&&<REQDetail req={selected} onClose={()=>setSelected(null)} />}
    </div>
  );
}

function REQForm({ onClose, onSubmit }) {
  const [f,setF]=useState({description:"",quantity:"",unit:"Unidad",requiredDate:"",supplier:"",priority:"normal"});
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  const valid=f.description&&f.quantity&&f.requiredDate;
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head">
          <div><div className="mod-title">Nueva Requisición de Compra</div><div style={{fontSize:11,color:B.gray3,marginTop:2}}>Se generará un número PO automáticamente</div></div>
          <button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button>
        </div>
        <div style={{display:"grid",gap:14}}>
          <div><label className="lbl">Descripción del ítem / servicio *</label><input className="inp" value={f.description} onChange={e=>set("description",e.target.value)} placeholder="Ej: Válvulas de control de presión 2 pulgadas…" /></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div><label className="lbl">Unidad de medida</label><select className="sel" value={f.unit} onChange={e=>set("unit",e.target.value)}>{UNITS.map(u=><option key={u}>{u}</option>)}</select></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div><label className="lbl">Proveedor sugerido (opcional)</label><input className="inp" value={f.supplier} onChange={e=>set("supplier",e.target.value)} placeholder="Nombre del proveedor" /></div>
          </div>
          <div>
            <label className="lbl">Prioridad</label>
            <div style={{display:"flex",gap:8}}>
              {Object.entries(PRIORITY).map(([k,p])=>(
                <button key={k} onClick={()=>set("priority",k)} style={{flex:1,padding:"10px",border:`1.5px solid ${f.priority===k?p.color:B.gray1}`,borderRadius:7,cursor:"pointer",background:f.priority===k?p.bg:B.offwhite,color:f.priority===k?p.color:B.gray3,fontFamily:"'Montserrat'",fontSize:11,fontWeight:700,transition:"all .15s",letterSpacing:.3}}>
                  {p.dot} {p.label}
                </button>
              ))}
            </div>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:22,paddingTop:16,borderTop:`1px solid ${B.gray1}`}}>
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-gold" disabled={!valid} style={{opacity:valid?1:0.45}} onClick={()=>onSubmit(f)}>Generar PO →</button>
        </div>
      </div>
    </div>
  );
...
function REQDetail({ req, onClose }) {
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su" style={{maxWidth:640}}>
        <div className="mod-head">
          <div><span className="bpo">{req.id}</span><div style={{fontFamily:"'Montserrat'",fontSize:16,fontWeight:800,color:B.carbon,marginTop:8}}>{req.description}</div></div>
          <button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button>
        </div>
        <div className="card" style={{padding:"14px 16px",marginBottom:14}}><ProgressBar stage={req.stage} /></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          {[["Cantidad",`${req.quantity} ${req.unit}`],["Fecha requerida",new Date(req.requiredDate).toLocaleDateString("es-ES",{day:"2-digit",month:"long",year:"numeric"})],["Proveedor sugerido",req.supplier||"—"],["Prioridad",PRIORITY[req.priority].label]].map(([k,v])=>(
            <div key={k} style={{background:B.offwhite,borderRadius:8,padding:"10px 14px",border:`1px solid ${B.gray1}`}}>
              <div style={{fontSize:10,color:B.gray3,fontFamily:"'Montserrat'",fontWeight:700,letterSpacing:.5,marginBottom:3}}>{k.toUpperCase()}</div>
              <div style={{fontSize:13,color:B.carbon,fontWeight:600}}>{v}</div>
            </div>
          ))}
        </div>
        <div>
          <div style={{fontSize:10,color:B.gray3,fontFamily:"'Montserrat'",fontWeight:700,letterSpacing:1,marginBottom:10}}>HISTORIAL DE MOVIMIENTOS</div>
          {req.history.map((h,i)=>{
            const s=STAGES.find(s=>s.id===h.stage);
            return(<div key={i} style={{display:"flex",gap:10,marginBottom:8,alignItems:"flex-start"}}>
              <div style={{width:8,height:8,borderRadius:"50%",background:s?.color??B.gray3,marginTop:4,flexShrink:0}}/>
              <div><div style={{fontSize:12,color:B.carbon,fontWeight:500}}>{h.note}</div><div style={{fontSize:10,color:B.gray3}}>{new Date(h.date).toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}</div></div>
            </div>);
          })}
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:18}}><button className="btn btn-ghost" onClick={onClose}>Cerrar</button></div>
      </div>
    </div>
  );
}
...
function RFQPage({ ctx }) {
  ...
  const handleCreate=async(data)=>{
    const n=ctrRFQ+1;
    const rfq={...data,id:genId("RFQ",n),createdAt:new Date().toISOString(),status:"enviada",responses:data.supplierIds.map(sid=>({supplierId:sid,status:"pendiente",price:null,deliveryDays:null,paymentTerms:"contado",notes:"",respondedAt:null}))};
  ...
  const handleAddSup=async(data)=>{
    const ns={...data,id:`S${String(sups.length+1).padStart(3,"0")}`,rating:4.0,active:true};
    await saveSups([...sups,ns]);
    setShowAddSup(false); showToast(`${data.name} agregado`);
  };
    ...
    <div className="fi">
      <div className="ph">
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{display:"flex",gap:2,background:B.offwhite,padding:3,borderRadius:7,border:`1px solid ${B.gray1}`}}>
            {[["rfqs","Cotizaciones"],["sups","Proveedores"]].map(([id,l])=><button key={id} onClick={()=>setTab(id)} style={{padding:"6px 14px",border:"none",borderRadius:5,background:tab===id?B.white:"transparent",color:tab===id?B.carbon:B.gray3,fontFamily:"'Montserrat'",fontSize:11,fontWeight:700,cursor:"pointer",boxShadow:tab===id?"0 1px 3px rgba(0,0,0,.08)":"none"}}>{l}</button>)}
          </div>
          {tab==="rfqs"&&<button className="btn btn-gold" onClick={()=>setShowNew(true)}>+ Nueva RFQ</button>}
          {tab==="sups"&&<button className="btn btn-dark" onClick={()=>setShowAddSup(true)}>+ Proveedor</button>}
        </div>
      </div>
      <div className="pb">
        <div className="gold-line" />
          ...
          <div className="card" style={{overflow:"hidden"}}>
            ...
            rfqs.map(rfq=>{
              ...
              return(<div key={rfq.id} className="trow" onClick={()=>setSelected(rfq)}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,color:B.carbon,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r?.description??"—"}</div>
                  <div style={{fontSize:11,color:B.gray3}}>Vence: {new Date(rfq.dueDate).toLocaleDateString("es-ES",{day:"2-digit",month:"short"})} · {tot} proveedores contactados</div>
                </div>
                <div style={{width:110}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:B.gray3,marginBottom:3,fontFamily:"'Montserrat'",fontWeight:600}}><span>Respuestas</span><span style={{color:resp===tot?"#5AAD7A":B.goldD}}>{resp}/{tot}</span></div>
                  <div style={{height:4,background:B.gray1,borderRadius:2,overflow:"hidden"}}><div style={{width:`${tot?resp/tot*100:0}%`,height:"100%",background:resp===tot?"#5AAD7A":B.gold,borderRadius:2}}/></div>
                </div>
                <span style={{color:B.gray2,fontSize:14}}>›</span>
              </div>);
            })}
          ...
          <div className="card" style={{overflow:"hidden"}}>
            ...
            {sups.map(s=>(
              <div key={s.id} className="trow" style={{cursor:"default"}}>
                <div style={{width:36,height:36,borderRadius:8,background:B.goldBg,border:`1px solid ${B.goldBd}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>🏢</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,color:B.carbon,fontWeight:600}}>{s.name}</div>
                  <div style={{fontSize:11,color:B.gray3}}>{s.contact} · {s.email}</div>
                </div>
                <span className="chip" style={{background:B.goldBg,color:B.goldD,border:`1px solid ${B.goldBd}`}}>{s.category}</span>
                <Stars r={s.rating} />
              </div>
            ))}
          </div>
      ...
      {showAddSup&&<AddSupForm onClose={()=>setShowAddSup(false)} onSubmit={handleAddSup} />}
    </div>
  );
}
...
function RFQForm({ eligible, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div className="mod-title">Nueva Solicitud de Cotización (RFQ)</div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        {eligible.length===0?<Empty icon="💬" msg="Sin POs disponibles" sub="Crea una requisición primero" />:(
          <div style={{display:"grid",gap:14}}>
            ...
            <div><label className="lbl">Notas / especificaciones técnicas</label><textarea className="ta" value={f.notes} onChange={e=>set("notes",e.target.value)} /></div>
            <div>
            ...
            return(<div key={res.supplierId} style={{background:B.offwhite,border:`1px solid ${B.gray1}`,borderRadius:10,padding:"12px 16px"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:isRec?12:0}}>
                ...
                {res.status==="respondida"?(
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <span className="chip" style={{background:B.goldBg,color:B.goldD}}>⏳ Pendiente</span>
...
function AddSupForm({ onClose, onSubmit }) {
  const [f,setF]=useState({name:"",contact:"",email:"",phone:"",category:"Materiales"});
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  const valid=f.name&&f.contact&&f.email;
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su" style={{maxWidth:500}}>
        <div className="mod-head"><div className="mod-title">Agregar Proveedor</div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div><label className="lbl">Teléfono</label><input className="inp" value={f.phone} onChange={e=>set("phone",e.target.value)} /></div>
          </div>
          <div><label className="lbl">Categoría</label><select className="sel" value={f.category} onChange={e=>set("category",e.target.value)}>{["Materiales","Equipos","Servicios","Repuestos","Consumibles","Transporte","Otro"].map(c=><option key={c}>{c}</option>)}</select></div>
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:20,paddingTop:14,borderTop:`1px solid ${B.gray1}`}}>
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-gold" disabled={!valid} style={{opacity:valid?1:0.45}} onClick={()=>onSubmit(f)}>Agregar proveedor →</button>
        </div>
      </div>
    </div>
  );
          ...
          cmps.map(c=>{
            ...
            return(<div key={c.id} className="trow" onClick={()=>setSelected(c)}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:B.gray3}}>Proveedor recomendado: <strong style={{color:"#5AAD7A"}}>{winner?.name??"—"}</strong></div>
              </div>
              ...
              <span className="chip" style={{background:c.status==="aprobado"?"rgba(90,173,122,.12)":B.goldBg,color:c.status==="aprobado"?"#5AAD7A":B.goldD}}>{c.status==="aprobado"?"✓ Aprobado":"⏳ Pendiente"}</span>
              <span style={{color:B.gray2,fontSize:14}}>›</span>
            </div>);
          })}
        </div>
...
function CMPForm({ readyRFQs, reqs, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div className="mod-title">Nuevo Cuadro Comparativo</div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:14}}>
      ...
      <div className="mod su" style={{maxWidth:860}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(${cmp.scored?.length??1},1fr)`,gap:12,marginBottom:18}}>
          {(cmp.scored??[]).map((r,i)=>{
            ...
            return(<div key={r.supplierId} onClick={()=>cmp.status==="pendiente"&&setOverride(r.supplierId)} style={{padding:"16px",borderRadius:10,background:isWin?"rgba(90,173,122,.06)":B.offwhite,border:`2px solid ${isWin?"#5AAD7A":B.gray1}`,cursor:cmp.status==="pendiente"?"pointer":"default",transition:"all .2s",boxShadow:isWin?"0 4px 16px rgba(90,173,122,.15)":"none"}}>
              ...
              {cmp.status==="pendiente"&&<div style={{textAlign:"center",marginTop:10,fontSize:10,color:isWin?"#5AAD7A":B.gray3,fontFamily:"'Montserrat'",fontWeight:700}}>{isWin?"✓ SELECCIONADO":"Clic para seleccionar"}</div>}
            </div>);
          })}
        </div>
...
function POPage({ ctx }) {
  ...
  const stMap={pendiente:{l:"Pend. Aprobación",c:B.goldD},aprobado:{l:"Aprobada",c:"#5AAD7A"},rechazado:{l:"Rechazada",c:"#D4745A"}};
  return(
    <div className="fi">
      <div className="ph">
      ...
      <div className="pb">
        <div className="gold-line" />
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          {[["Pend. Aprobación",orders.filter(o=>o.approvalStatus==="pendiente").length,B.goldD],["Aprobadas",orders.filter(o=>o.approvalStatus==="aprobado").length,"#5AAD7A"],["Enviadas",orders.filter(o=>o.sentToSupplier).length,"#5B9BD5"]].map(([l,v,c])=>(
...
function POForm({ readyCMPs, cmps, reqs, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div><div className="mod-title">Emitir Orden de Compra</div><div style={{fontSize:11,color:B.gray3,marginTop:2}}>Los datos del ítem y proveedor se cargan automáticamente desde el comparativo</div></div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:14}}>
          ...
          {cmp&&<div style={{background:B.goldBg,border:`1px solid ${B.goldBd}`,borderRadius:8,padding:"12px 16px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
              {[["Número PO",req?.id],["Proveedor",sup?.name],["Total OC",`$${total.toLocaleString()}`]].map(([k,v])=><div key={k}><div style={{fontSize:9,color:B.goldD,fontFamily:"'Montserrat'",fontWeight:700,marginBottom:2}}>{k}</div><div style={{fontSize:13,color:B.carbon,fontWeight:700}}>{v}</div></div>)}
            </div>
          </div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div style={{padding:"10px 14px",background:"rgba(201,168,76,.08)",border:`1px solid ${B.goldBd}`,borderRadius:8,fontSize:12,color:B.goldD,fontFamily:"'Montserrat'",fontWeight:600}}>
            ⚠️ Al crear la OC quedará en estado Pendiente de Aprobación hasta que un gerente la apruebe.
          </div>
          <div style={{display:"flex",justifyContent:"flex-end",gap:10,paddingTop:14,borderTop:`1px solid ${B.gray1}`}}>
            <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function DELPage({ ctx }) {
  ...
  const handleCreate=async(data)=>{
    const d={...data,id:genId("DEL",dels.length+1),createdAt:new Date().toISOString(),status:"en_camino",partials:data.partials.map((p,i)=>({...p,id:`P${i+1}`,received:false,receivedAt:null,receivedQty:0})),events:[{date:new Date().toISOString(),note:"Plan de delivery registrado",icon:"📋"}]};
  ...
  const stC={en_camino:"#4AADA0",parcial:B.goldD,completado:"#5AAD7A",retrasado:"#D4745A"};
...
function DELForm({ orders, reqs, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div className="mod-title">Registrar Plan de Delivery</div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:14}}>
...
function GRForm({ deliveries, reqs, orders, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div><div className="mod-title">Nueva Nota de Recepción (GR)</div><div style={{fontSize:11,color:B.gray3,marginTop:2}}>Verificación contra Orden de Compra</div></div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:14}}>
  ...
  return(
    <div className="fi">
      ...
      <div className="pb">
        <div className="gold-line" />
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          {[["Listos para pagar",pays.filter(p=>p.status==="listo").length,"#5B9BD5"],["Pagados",pays.filter(p=>p.status==="pagado").length,"#5AAD7A"],["Total pagado",`$${totalPagado.toLocaleString()}`,B.goldD]].map(([l,v,c])=>(
            <div key={l} className="stat" style={{"--sc":c}}><div className="stat-val">{v}</div><div className="stat-lbl">{l}</div></div>
          ))}
        </div>
        <div className="card" style={{overflow:"hidden"}}>
...
function PAYForm({ rcvs, orders, reqs, sups, onClose, onSubmit }) {
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su">
        <div className="mod-head"><div><div className="mod-title">Preparar Paquete de Pago</div><div style={{fontSize:11,color:B.gray3,marginTop:2}}>3-Way Match: OC + GR + Factura</div></div><button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button></div>
        <div style={{display:"grid",gap:14}}>
          ...
          {gr&&<div style={{background:B.goldBg,border:`1px solid ${B.goldBd}`,borderRadius:8,padding:"12px 16px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[["Proveedor",sup?.name],["Monto OC",`$${poAmt.toLocaleString()}`],["GR",gr.result==="conforme"?"✅ Conforme":"⚠️ No conforme"]].map(([k,v])=><div key={k}><div style={{fontSize:9,color:B.goldD,fontFamily:"'Montserrat'",fontWeight:700,marginBottom:2}}>{k.toUpperCase()}</div><div style={{fontSize:12,color:B.carbon,fontWeight:700}}>{v}</div></div>)}
            </div>
          </div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          {f.invoiceAmount&&(
            <div style={{padding:"14px 16px",background:matchOk?"rgba(90,173,122,.06)":"rgba(201,168,76,.06)",border:`1px solid ${matchOk?"rgba(90,173,122,.25)":B.goldBd}`,borderRadius:10}}>
              <div style={{fontFamily:"'Montserrat'",fontSize:13,fontWeight:800,color:matchOk?"#5AAD7A":B.goldD,marginBottom:8}}>{matchOk?"✅ 3-Way Match aprobado":"⚠️ Match con diferencias — se documenta para CxP"}</div>
              <div style={{display:"flex",gap:10}}>
  ...
  return(
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mod su" style={{maxWidth:640}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[["Proveedor",sup?.name],["N° Factura",pay.invoiceNumber],["Monto a pagar",`$${parseFloat(pay.invoiceAmount).toLocaleString()}`],["Método de pago",pay.paymentMethod],["Vencimiento",pay.dueDate||"—"],["Notas CxP",pay.notes||"—"]].map(([k,v])=>(
            <div key={k} style={{background:B.offwhite,borderRadius:8,padding:"10px 14px",border:`1px solid ${B.gray1}`}}>
              <div style={{fontSize:9,color:B.gray3,fontFamily:"'Montserrat'",fontWeight:700,letterSpacing:.5,marginBottom:3}}>{k.toUpperCase()}</div>
              <div style={{fontSize:12,color:B.carbon,fontWeight:600}}>{v}</div>
            </div>
